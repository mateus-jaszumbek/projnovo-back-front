...
SELECT gen_random_uuid(), t.""EmpresaId"", t.""Categoria"", true, now(), now()
FROM (
    SELECT DISTINCT ""EmpresaId"", trim(""Categoria"") AS ""Categoria""
    FROM pecas
    WHERE ""Categoria"" IS NOT NULL AND trim(""Categoria"") <> ''
) t;

UPDATE pecas p
SET ""CategoriaPecaId"" = c.""Id""
FROM categorias_peca c
WHERE c.""EmpresaId"" = p.""EmpresaId"" AND c.""Nome"" = trim(p.""Categoria"");
");
