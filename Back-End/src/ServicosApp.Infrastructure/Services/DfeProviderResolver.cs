...
public class DfeProviderResolver : IDfeProviderResolver
{
    private readonly IReadOnlyDictionary<string, IDfeProviderClient> _providers;

    public DfeProviderResolver(IEnumerable<IDfeProviderClient> providers)
    {
    ...
    public IDfeProviderClient Resolve(ConfiguracaoFiscal configuracaoFiscal, CredencialFiscalEmpresa? credencial)
    {
        ...
        throw new InvalidOperationException(
            $"Provedor fiscal '{providerCode}' não está registrado para DF-e. Registrados: {FormatRegisteredProviders(_providers.Keys)}.");
    }
        ...
        IEnumerable<string> registeredProviders)
    {
        ...
        throw new InvalidOperationException(
            "Nenhum provedor fiscal foi configurado para DF-e. Configure o provedor na credencial fiscal da empresa ou na configuração fiscal.");
    }
