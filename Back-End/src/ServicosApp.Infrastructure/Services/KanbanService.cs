...
public class KanbanService : IKanbanService
{
    ...
    public async Task<KanbanConfiguracaoColunaDto> CriarColunaPublicaAsync(Guid empresaId, CreateKanbanPublicoColunaDto dto, CancellationToken cancellationToken = default)
    {
        ...
        if (dto.EtapaFinal)
        {
            ...
            if (jaExisteMesmoTipo)
                throw new InvalidOperationException($"J� existe uma coluna final do tipo {tipoFinalizacao}.");
        }
    ...
    public async Task<bool> ExcluirColunaPublicaPermanentementeAsync(Guid empresaId, Guid colunaId, CancellationToken cancellationToken = default)
    {
        ...
        if (coluna.Sistema || coluna.EtapaFinal)
            throw new InvalidOperationException("Essa coluna n�o pode ser exclu�da definitivamente.");

        if (coluna.Ativa)
            throw new InvalidOperationException("Desative a coluna antes de exclu�-la definitivamente.");
        ...
        if (possuiHistorico)
            throw new InvalidOperationException("Essa coluna j� faz parte do hist�rico das OS e s� pode permanecer desativada.");
    ...
    public async Task<bool> ExcluirColunaPrivadaPermanentementeAsync(Guid empresaId, Guid usuarioId, Guid colunaId, CancellationToken cancellationToken = default)
    {
        ...
        if (coluna.Sistema)
            throw new InvalidOperationException("As colunas padr�o n�o podem ser exclu�das definitivamente.");
    ...
    public async Task<KanbanPrivadoCardDto> CriarTarefaPrivadaAsync(Guid empresaId, Guid usuarioId, CreateKanbanTarefaPrivadaDto dto, CancellationToken cancellationToken = default)
    {
        ...
        if (dto.KanbanColunaId.HasValue)
        {
            ...
            if (!colunaValida)
                throw new InvalidOperationException("Coluna inv�lida para o kanban privado.");
